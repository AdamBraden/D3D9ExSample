...
//Globals for managing Dynamic Dependencies
...
// Function Prototypes
//--------------------------------------------------------------------------------------
HRESULT SetupDynamicDependencyToDirectXFrameworkPackage();
void CleanUpDynamicDependencyToDirectXFrameworkPackage();
                       ...
                       int       nCmdShow )
{
    ...
    // Initialize bootstrapper to find WinAppSDK and Dynamic Dependencies
    ...
    MessageBox(NULL, L"calling dyn dep", L"D3D9Ex sample", MB_OK );
    // Use Dynamic Dependencies api to use DirectX framework package
    if (FAILED(SetupDynamicDependencyToDirectXFrameworkPackage()))
        return 10;
...
exit:
    ...
    // Clean up Dynamic Dependencies reference to DirectX framework package
    CleanUpDynamicDependencyToDirectXFrameworkPackage();
...
//--------------------------------------------------------------------------------------
// Register a Dynamic Dependency to the DirectX framework package
//--------------------------------------------------------------------------------------
 HRESULT SetupDynamicDependencyToDirectXFrameworkPackage()
{

     // use powershell "get-appxpackage *directx* to find details
     PCWSTR packageFamilyName = L"Microsoft.DirectXRuntime_8wekyb3d8bbwe";
     PACKAGE_VERSION minVersion{};
     minVersion.Major = 9;
     minVersion.Minor = 29;
     minVersion.Build = 1974;
     minVersion.Revision = 0;

     // set various filters and lifetime artifacts
     ...
     RETURN_IF_FAILED(MddTryCreatePackageDependency(nullptr, packageFamilyName, minVersion, 
         architectureFilter, lifetimeKind, nullptr, createOptions, &packageDependencyId));
     ...
     wprintf(L"Managing DirectX via %ls", packageFullName.get());
}

//--------------------------------------------------------------------------------------
// Remove Dynamic Dependency to the DirectX framework package
//--------------------------------------------------------------------------------------
 void CleanUpDynamicDependencyToDirectXFrameworkPackage()
 {
     ...
     // Remove the install time reference - ideally this is part of your Installer
